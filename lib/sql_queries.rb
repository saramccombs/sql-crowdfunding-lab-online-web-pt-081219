...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS Amount_Over_Goal FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING (SUM(pledges.amount) >= projects.funding_goal)"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_summed_amount
  "SELECT users.name, SUM(pledges.amount) AS pledged_amount FROM users INNER JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY pledged_amount ASC"
end
